...
public:
  ...
  AVL(const AVL& other){
    cmpfn = other.cmpfn;
    my_size = other.my_size; //TODO change 0 to other.my_size
  ...
  T getLargest() const{
   return getLargest(root); //TODO change Smallest to Largest
  }
...
//    }else if( (*cmpfn)(thing, node->data) < 1){
  ...
  unsigned int getHeight(Node<T>* node) const{
    if(node == NULL){
      return 0; //TODO change from -1 to 0
    }else{
      return node->height;
    }
  }
    ...
    if(node->data == thing){ // found it!
      if(node == root){ // If we deleted the root node we can't fix the parent
        if(!node->left && !node->right){// leaf node
          delete node;
          root = NULL;
          my_size = my_size - 1; //TODO change -2 to -1
      ...
      if(!node->left && !node->right){// leaf node
        ...
        T move_up = getLargest(node->left);   // Find largest in left subtree //TODO change right to left
        remove(node->left, move_up);          // Remove it!
        node->data = move_up;                 // Put it here
        fixHeight(node); // do full height recalc since we don't know where the node was from
      }else if(node->left){ // right must be null  1 child

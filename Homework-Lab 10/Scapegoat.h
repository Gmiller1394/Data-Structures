...
/*
   Scapegoat Binary Search Tree class. Homework 10
 */
#include <ostream>
#include <string>
#include <sstream>
...
private:
  ...
  Node<T>* replicateFast(Node<T>* other_node){
    ...
    if(other_node == NULL){
      ...
      return new Node<T>(other_node,
        ...
        replicateFast(other_node->right), //TODO change left to right
        other_node->data);
    }
  }
  ...
  bool find(Node<T>* node, const T& thing) const{
    if(node == NULL){
      ...
      return find(node->right, thing); //TODO change left to right
    };
  }
  ...
  T getSmallest(Node<T>* node) const{
    ...
    while(node->left != NULL){ //TODO change from right to left
      node = node->left; //TODO change from right to left
    }
    return node->data;
  }
  ...
  bool isFull(Node<T>* node) const{
    if(node == NULL){
      ...
      return isFull(node->left) && isFull(node->right); //TODO change || to &&
    }else if(!node->left && !node->right){
      return true;
    }
